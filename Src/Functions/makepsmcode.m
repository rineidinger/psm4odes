...
%    newfunc: function handle to the file created or overwritten
%    valuelist: all intermediate values computed in f(t0,y0) evaluation.
%    fvalue: f(t0,y0) final value, may be a vector.
%    location: index of final value(s) within the valuelist.
% by Richard Neidinger 1/8/21; rev 10/26/22(allowed DE = constant); 2/15/25
...
if ~isa(h,'trace')
    disp(['Overloading ',func2str(fhandle),' FAILED, try preallocating output with input (not zeros).']);
end
...
for i = 1:numDEs
    if isnan(location(i)) % trivial case of DE y(i)' = constant
...
% Write MATLAB .m program file for series recurrence code
funcname = func2str(fhandle);
if funcname(1) == '@' % anonymous function
    % Write MATLAB .m file for explicit version of the anonymous function
    fileID1 = fopen('tempDEfunc.m','wt');
    closeparens = strfind(funcname,')'); % location(s) of ')' in funcname
    argend = closeparens(1); % first ')' ends the argument half
    fprintf(fileID1,'%s\n',['function dydt = tempDEfunc',funcname(2:argend)]);
    fprintf(fileID1,'%s\n',['dydt = ',funcname(argend+1:end),';']);
    fclose(fileID1);
    funcname = 'tempDEfunc';
end
...
fprintf(fileID,'%s\n',['% Automatically generated by makepsmcode.m, ',char(datetime),'.']);
