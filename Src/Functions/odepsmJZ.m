...
while hsign* (t(k) - tend) < 0
    ...
    scale = max(1,norm(y(:,k),inf)); % scale for a y value larger than 1
    maxcoefs = max(abs(coefs(:,2:end)),[],1); % max across components of each coef
    % implementation note: vpa coefs work using max(abs but not vecnorm 
    roottests = (scale./maxcoefs).^(1./(1:deg));
